...
function Get-Components {
    ...
    if ($env:SBX_DEV_COMPONENTS) {
        $requested = $env:SBX_DEV_COMPONENTS.ToLowerInvariant()
        if ($requested -notin @('both', 'client', 'server')) {
            throw "SBX_DEV_COMPONENTS must be both, client or server, got: $requested"
        }
        return $requested
    }
    return 'both'
...
function Get-ReleaseTag {
    $requested = if ($Version) { $Version } else { $env:SBX_DEV_VERSION }
    if ($requested) {
...
function Get-InstallDirectory {
    $target = if ($Dir) { $Dir } elseif ($env:SBX_DEV_INSTALL_DIR) { $env:SBX_DEV_INSTALL_DIR }
    else { Join-Path $env:LOCALAPPDATA 'sbx-dev\bin' }
...
# Test-OurClient reports whether a path holds an sbx-dev client rather than the
# real Docker Sandboxes CLI, so an upgrade can be told apart from a collision.
function Test-OurClient {
    param([string]$Path)
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) { return $false }
    try {
        $previous = $env:SBX_DEV_PRINT_VERSION
        $env:SBX_DEV_PRINT_VERSION = '1'
        $output = & $Path 2>$null | Select-Object -First 1
        return ($output -like 'sbx-dev client *')
    } catch {
        return $false
    } finally {
        $env:SBX_DEV_PRINT_VERSION = $previous
    }
}
...
function Install-Binary {
    ...
    try {
        ...
        throw "could not write ${target}: $($_.Exception.Message). Stop any running sbx-dev and retry."
    }
...
$tempDir = Join-Path ([IO.Path]::GetTempPath()) ("sbx-dev-" + [Guid]::NewGuid().ToString('N'))
...
try {
    Write-Host "sbx-dev $tag for windows/$arch into $installDir"
    ...
    if ($components -in @('both', 'server')) {
        $extracted = Get-Archive -Name "sbx-dev_${releaseVersion}_windows_${arch}.zip" `
            -Tag $tag -TempDir $tempDir -ChecksumFile $checksumFile
        Install-Binary -SourceDir $extracted -Name 'sbx-dev.exe' -Destination $installDir
    ...
    if ($components -in @('both', 'client')) {
        ...
        if ((Test-Path -LiteralPath $target) -and -not (Test-OurClient -Path $target)) {
            if ($Force) {
                Write-Warning "replacing $target, which is not an sbx-dev client"
            } elseif ($components -eq 'client') {
                throw "$target exists and is not an sbx-dev client; pass -Force to replace it or -Dir to install elsewhere"
            } else {
                Write-Warning "skipping the client: $target exists and is not an sbx-dev client"
